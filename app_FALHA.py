...
def load_template(feature, template_name):
    """
    Carrega um template pré-configurado
    
    Args:
        feature (str): Nome da funcionalidade
        template_name (str): Nome do template
        
    Returns:
        dict: Dados do template selecionado ou None se não existir
    """
    if feature in template_library and template_name in template_library[feature]:
        return template_library[feature][template_name]
    return None

# Componente de painel de histórico expandido
def render_history_panel():
    """
    Renderiza um painel de histórico expandido com filtros e mais detalhes
    """
    with st.expander("Histórico de Comunicações", expanded=st.session_state.history_expanded):
        # Filtros para o histórico
        col1, col2 = st.columns(2)
        with col1:
            filter_feature = st.selectbox(
                "Filtrar por tipo",
                ["Todos"] + list(feature_options.keys()),
                key="history_filter_feature"
            )
        ...
        if filter_feature != "Todos":
            filtered_history = [item for item in filtered_history if item['feature'] == filter_feature]
        
        # Aplicar ordenação
        if sort_order == "Mais antigo primeiro":
            filtered_history = list(reversed(filtered_history))
        
        # Exibir histórico com mais detalhes
        if not filtered_history:
            st.info("Nenhum item no histórico corresponde aos filtros aplicados.")
        else:
            for i, item in enumerate(filtered_history):
                with st.container():
                    ...
                    with col2:
                        st.caption(f"**{item['feature']}**")
                    with col3:
                        st.caption(f"**{'🔄 Cache' if 'from_cache' in item and item['from_cache'] else '🔧 ' + item['model']}**")
                    
                    # Mostrar input resumido e output
                    st.text_area("Input", item['input'], height=50, key=f"hist_input_{i}")
                    
                    # Ações
                    col1, col2 = st.columns([4, 1])
                    with col1:
                        view_output = st.expander("Ver conteúdo completo", expanded=False)
                        with view_output:
                            st.markdown(item['output'])
                    
                    with col2:
                        if st.button(f"Carregar ↩️", key=f"load_hist_{i}"):
...
# Seleção de modelo mais proeminente
def model_selector():
    """
    Componente para seleção de modelo de IA
    """
    ...
    model_choice = st.radio(
        ...
        [
            ...
            "GPT-4 (Mais capaz, maior custo)"
        ],
        help="O GPT-3.5 é adequado para a maioria das tarefas comuns. Use GPT-4 para comunicações mais complexas ou críticas."
    )
    
    # Atualizar escolha na sessão
    st.session_state.model_choice = "gpt-3.5-turbo" if "GPT-3.5" in model_choice else "gpt-4"
    
    # Exibir informação sobre tokens
    if "GPT-4" in model_choice:
        st.info("⚠️ O GPT-4 consome significativamente mais tokens do seu limite.")
    st.markdown('</div>', unsafe_allow_html=True)

# Componente de exportação melhorado
def enhanced_export_options(generated_content, feature_name):
    """
    Opções de exportação aprimoradas para diferentes formatos
    
    Args:
        generated_content (str): Conteúdo gerado para exportar
        feature_name (str): Nome da funcionalidade atual
    """
    ...
    export_format = st.radio(
        "Formato:",
        ["Texto (.txt)", "Word (.docx)", "Markdown (.md)"],
        horizontal=True,
    ...
    filename_base = st.text_input(
        "Nome do arquivo:",
        value=f"{feature_name.lower().replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}",
        key="export_filename"
    )
    
    # Botões de download baseados no formato
    if export_format == "Texto (.txt)":
        ...
        # Para MD, mantemos o conteúdo como está se já estiver em markdown
        st.download_button(
    ...
    # Botão adicional para copiar para a área de transferência
    st.markdown("""
    <button onclick="navigator.clipboard.writeText(`""" + generated_content.replace("`", "\\`").replace("'", "\\'").replace('"', '\\"') + """`)" style="background-color: #4CAF50; color: white; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; margin-top: 10px; width: 100%;">
...
def template_selector(feature):
    """
    Componente para seleção e aplicação de templates
    
    Args:
        feature (str): Nome da funcionalidade atual
        
    Returns:
        dict or None: Template selecionado ou None se nenhum
    """
    if feature not in template_library:
        return None
    
    available_templates = list(template_library[feature].keys())
    
    if not available_templates:
        return None
    
    with st.expander("📚 Templates Pré-configurados", expanded=False):
        st.markdown("Selecione um modelo pré-configurado para acelerar seu trabalho:")
        
        template_choice = st.selectbox(
            "Template:",
            ["Selecione um template..."] + available_templates,
            key="template_selector"
        )
        
        if template_choice and template_choice != "Selecione um template...":
            template = template_library[feature][template_choice]
            st.markdown(f"**{template_choice}**: {template['description']}")
            
            if st.button("Aplicar este template"):
                return template
...
# Função completa para renderizar a barra lateral
def render_sidebar():
    with st.sidebar:
