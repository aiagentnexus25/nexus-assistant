...
<style>
    ...
    * {
    ...
    .button-purple:hover, .button-orange:hover, .button-teal:hover {
...
def setup_sidebar():
    ...
    with st.sidebar:
        ...
        st.markdown('<div style="background:#F5F5F7; border-radius:8px; padding:10px; margin:10px 0; border:1px solid #E2E8F0;">', unsafe_allow_html=True)
    ...
    if not st.session_state.current_feature:
        ...
        if st.session_state.token_count >= TOKEN_LIMIT:
            ...
            with st.form(key=f"{current_feature}_form"):
                ...
                with col1:
                    st.markdown(f'<div class="button-purple">Gerar {current_feature}</div>', unsafe_allow_html=True)
                    submit_button = st.form_submit_button("Gerar", type="primary")
                
                with col2:
                    st.markdown('<div class="button-orange">Visualizar Exemplo</div>', unsafe_allow_html=True)
                    view_example = st.form_submit_button("Exemplo")
                
                with col3:
                    st.markdown('<div class="button-teal">Limpar</div>', unsafe_allow_html=True)
