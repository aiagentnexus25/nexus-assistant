...
def get_relevant_scenarios(scenarios_data, context, audience=None, complexity_level="todos"):
    """
    Retorna cenários relevantes com base no contexto, público e nível de complexidade.
    """
    all_relevant = []
    ...
    for scenario_type, scenarios in scenarios_data.items():
        ...
        for scenario in scenarios.get("cenarios", []):
            # Verifica nível de complexidade
            if complexity_level != "todos" and scenario.get("nivel") != complexity_level:
                continue
...
if 'complexity_level' not in st.session_state:
    st.session_state.complexity_level = "todos"
    ...
    if not st.session_state.current_feature:
        ...
        if st.session_state.token_count >= TOKEN_LIMIT:
            ...
            with st.form(key=f"{current_feature}_form"):
                ...
                # Nível de complexidade para cenários (em todas as funcionalidades exceto PMBOK)
                if current_feature != "Consultor PMBOK 7":
                    complexity_level = st.select_slider(
                        "Nível de complexidade dos cenários", 
                        options=["básico", "intermediário", "avançado", "todos"],
                        value=st.session_state.complexity_level,
                        help="Selecione o nível de complexidade dos cenários de referência"
                    )
